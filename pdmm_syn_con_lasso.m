...
function output=pdmm_syn_con_lasso(Geograph,error_th,iteration_max,Pmax,c,lamda,flag)
...
while iteration <= iteration_max 
    for i=1:n
         ind_i=(i-1)*ni+1:i*ni;
        x_est(i,:)=(reshape(invH(i,:,:),u,u)*wthresh((H_matrix(ind_i,:).'*y_vec(ind_i)-(C_matrix(:,i).'*z_matrix).'),'s',lamda)).';
    end
      z_matrixTem=circshift(z_matrix,numEdge/2)+2*circshift(c*C_matrix*x_est,numEdge/2);              
      z_matrix=((1-theta)*z_matrixTem+theta*z_matrix);
